...
var config = require('./config').redisSocket;
...
const socketFunction = (server) => {
	...
	io.on('connection', function (socket) {
		...
		socket.on(Constant.SOCKET_CONNECT, (data) => {
			socket.join(data.room);
			const from_time = data.from + 1;
			//get all messages from database and respond to client.
			
		});

		socket.on("android_on_message", (data) => {
			socket.join(data);
			//
			socket.emit("android_on_server_message", "done");
		});
		socket.on("android_on_disconnect", (data) => {
			socket.emit("android_on_server_disconnect", "done");

		});
		socket.on("android_on_typing", (data) => {
			socket.emit("android_on_server_typing", "done");

		});
		socket.on("android_on_more_message", (data) => {
			socket.emit("android_on_server_more_message", "done");
